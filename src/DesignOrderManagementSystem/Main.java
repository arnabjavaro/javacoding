...
public class Main {
    ...
    private void runDeliveryFlow(ProductDeliverySystem productDeliverySystem, int userId){
       ...
       Warehouse warehouse = productDeliverySystem.getWarehouse(new NearestWarehouseSelectionStrategy());
        ...
        System.out.println("warehouse has products before ordering");
        for (int i = warehouse.inventory.productCategoryList.size() - 1; i >= 0; i--) {
            System.out.println(warehouse.inventory.productCategoryList.get(i).categoryName+"====> "+warehouse.inventory.productCategoryList.get(i).products.size());
